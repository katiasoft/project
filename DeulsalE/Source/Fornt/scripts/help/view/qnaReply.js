...
contents.answerForm = contents.querySelector('[rel="answerForm"]');

contents.answerForm.onsubmit = e => {
    e.preventDefault();
    if (contents.answerForm['content'].value === '') {
        dialogCover.show();
        dialogLayer.show({
            title: '경고',
            content: '내용을 입력해 주세요',
    ...
    formData.append('content', contents.answerForm['content'].value);
    formData.append('qnaIndex', contents.answerForm['qnaIndex'].value);

    xhr.open('POST', `/help/qna/view?index=${contents.answerForm['qnaIndex'].value}`);
